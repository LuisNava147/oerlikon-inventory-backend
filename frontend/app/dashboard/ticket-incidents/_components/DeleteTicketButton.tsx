...
export default function DeleteTicketButton({ticket}:{ticket:TicketIncident}){
    const { isOpen, onOpen, onOpenChange, onClose } = useDisclosure();
    const [error, setError] = useState<string | null>(null)

    const handleDelete = async () => {
        setError(null)
        const result = await deleteTicketIncident(ticket.ticketIncidentId)
            ...
            {(onClose) => (
                <form action={handleDelete}>
                    <ModalHeader className="flex flex-col gap-1 text-red-600 items-center">
                        ...
                        <span className="text-xl">¿Eliminar Ticket?</span>
                    </ModalHeader>
                    <ModalBody className="text-center">
                        <p className="text-slate-600">Estás a punto de eliminar el Ticket:</p>
                        ...
                        <p className="text-xs text-slate-400 mt-4">
                            Esta acción es irreversible y se perderá el historial de tickets.
                        </p>
                        {error && (
