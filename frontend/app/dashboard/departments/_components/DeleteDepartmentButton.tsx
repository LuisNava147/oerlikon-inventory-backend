...
import { deleteDepartment } from "@/actions/departments/department-delete";
...
export default function DeleteDepartmentButton({departments}:{departments:Deparment}){
    ...
    const handleDelete = async () => {
        setError(null)
        const result = await deleteDepartment(departments.departmentId)
        ...
        {(onClose) => (
            <form action={handleDelete}>
                <ModalHeader className="flex flex-col gap-1 text-red-600 items-center">
                    ...
                    <span className="text-xl">¿Eliminar Departamento?</span>
                </ModalHeader>
                <ModalBody className="text-center">
                    <p className="text-slate-600">Estás a punto de eliminar el departamento:</p>
