...
export default function DeleteMobileButton({device}:{device: Device}){
        ...
        {(onClose) => (
            <form action={handleDelete}>
                <ModalHeader className="flex flex-col gap-1 text-red-600 items-center">
                    ...
                    <span className="text-xl">¿Eliminar Equipo?</span>
                </ModalHeader>
                <ModalBody className="text-center">
                    <p className="text-slate-600">Estás a punto de eliminar el equipo:</p>
                    <div className="bg-red-50 p-3 rounded-lg border border-red-100 mt-2">
                        ...
                        <p className="text-xs text-slate-500 font-mono mt-1">
                            NUM. TELÉFONO: {device.employee?.employeePhoneNumber|| null}
                        </p>
                        <p className="text-xs text-slate-500 font-mono mt-1">
                            CUENTA: {device.deviceAccount || null}
                        </p>
                        <p className="text-xs text-slate-500 font-mono mt-1">
                            USUARIO: {device.employee?.employeeName || null} {device.employee?.employeeLastName || null}
                        </p>
                    </div>
