...
import { FileQuestionMark, LaptopMinimal, MonitorCog, Pencil } from "lucide-react";
import FormUpdateStatus from "./FormUpdateStatus";

export default function UpdateDeviceStatus({devices}:{devices:Device}){
    ...
    return(
        ...
        <Modal isOpen={isOpen} onOpenChange={onOpenChange} scrollBehavior="inside" backdrop="blur" isDismissable={false}>
            <ModalContent className="w-full h-[90vh] md:h-auto max-h-[90vh]">
                {
                    ()=>(
                        ...
                        <ModalBody className="py-1 overflow-y-auto font-bold">
                            <FormUpdateStatus devices={devices} onClose={onOpenChange}/>
                        </ModalBody>
                        </>
                    )
                }
