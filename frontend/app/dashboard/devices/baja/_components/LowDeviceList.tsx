...
import LowDeviceIcon from "./LowDeviceIcon";
import LowDeviceActions from "./LowDeviceActions";
...
export default function LowDeviceList({ devices }: {devices:Device[]}){
    ...
    const renderCell = (device: Device, columnKey: React.Key) => {
        switch (columnKey) {
                ...
                return (
                    <div className="flex items-center gap-3 min-w-[160px] ">
                        <div className=" flex items-start justify-start p-2 bg-red-50 rounded-md shrink-0">
                           <LowDeviceIcon type={device.deviceType}/>
                ...
                return (
                    <div className="flex flex-col  gap-1 min-w-[160px]">
                        ...
                        {device.employee?.employeePhoneNumber && (
                            <div className="text-xs text-slate-500 flex justify-between items-center w-full border-b border-slate-200 pb-1">
                            <span className="font-semibold text-slate-700 block">NUM. TELÉFONO: </span>
                            <span className="font-semibold text-slate-500 text-right ml-2 truncate font-medum">{device.employee.employeePhoneNumber}</span>
                        </div>
                        )}
                        {device.deviceAccount && (
                            <div className="text-xs text-slate-500 flex justify-between items-center w-full border-b border-slate-200 pb-1">
                            <span className="font-semibold text-slate-700 block">CUENTA: </span>
                            <span className="font-semibold text-slate-500 text-right ml-2 truncate font-medum">{device.deviceAccount}</span>
                        </div>
                        )}
                        {device.devicePassword && (
                            <div className="text-xs text-slate-500 flex justify-between items-center w-full border-b border-slate-200 pb-1">
                            <span className="font-semibold text-slate-700 block">CONTRASEÑA: </span>
                            <span className="font-semibold text-slate-500 text-right ml-2 truncate font-medum">{device.devicePassword}</span>
                        </div>
                        )}
                        {device.devicePin && (
                            <div className="text-xs text-slate-500 flex justify-between items-center w-full border-b border-slate-200">
            ...
            case "actions":
                return(
                    <LowDeviceActions devices={device}/>
                )
