...
export default function SearchPrinters() {
    ...
    const handleSearch = () => {
        if (!query.trim()) {
            ...
            router.push(`/dashboard/printers?q=${encodeURIComponent(query)}&f=${filter}`);
        }
    };
        ...
        <div className="flex flex-col md:flex-row gap-3 bg-white p-4 rounded-xl shadow-sm border border-slate-100 items-end md:items-center">
            ...
            <div className="w-full md:w-56">
                ...
                >
                    ...
                    <SelectItem key="type">Modelo</SelectItem>
                    <SelectItem key="location">Ubicación</SelectItem>
                    <SelectItem key="ip">Dirección IP</SelectItem>
                    <SelectItem key="sap">Nombre en SAP</SelectItem>
                </Select>
            </div>
